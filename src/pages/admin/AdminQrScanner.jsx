  ...
  return (
    <div className="min-w-0 rounded-lg border border-slate-200 bg-slate-50/60 p-3">
      <div className="flex items-center justify-between gap-2">
        ...
        <span className="rounded-full bg-orange-100 px-2 py-1 text-[10px] font-extrabold text-orange-700">
          Qty {qr.quantity ?? stickers.length}
        </span>
      </div>
      ...
      <div className="mt-2">
        ...
        {stickers.length === 0 ? (
          <p className="mt-1 text-xs text-slate-500">No stickers</p>
        ) : (
          <div className="mt-1.5 flex flex-wrap gap-1.5">
            {stickers.map((sticker, index) => (
              <span
                key={sticker._id || `${sticker.barcodeId}-${index}`}
                className="max-w-full break-all rounded-md border border-slate-200 bg-white px-2 py-1 font-mono text-[10px] text-slate-700"
              >
                {sticker.barcodeId || '—'}
              </span>
            ))}
