...
const links = {
  ...
  subadmin: [
    ...
    { to: '/sub-admin/users', label: 'Users' },
    { to: '/sub-admin/scanner', label: 'Scanner' },
    { to: '/sub-admin/qr-generator', label: 'QR Generator' },
  ],
  vendor: [
...
const portalName = (role) => (role === 'subadmin' ? 'Sub-Admin' : role.charAt(0).toUpperCase() + role.slice(1));
      ...
      <header className="border-b border-slate-200 bg-white">
        <div className="mx-auto flex max-w-7xl items-center justify-between gap-4 px-5 py-4">
          <div>
            ...
            <h1 className="text-lg font-bold text-slate-900">{portalName(user.role)} Portal</h1>
          </div>
          <div className="flex items-center gap-3">
            <div className="hidden text-right sm:block">
              <p className="text-sm font-medium text-slate-800">{user.email}</p>
              <p className="text-xs text-slate-500">{portalName(user.role)}</p>
            </div>
          ...
          <nav className="flex gap-2 overflow-x-auto md:flex-col">
            {(links[user.role] || []).map((link) => (
              <NavLink
                ...
                className={({ isActive }) => `whitespace-nowrap rounded-lg px-3 py-2 text-sm font-semibold ${isActive ? 'bg-blue-900 text-white' : 'text-slate-600 hover:bg-slate-100'}`}
              >
                {link.label}
              </NavLink>
            ))}
